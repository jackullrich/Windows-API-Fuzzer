...
int __cdecl main() {
  ...
  IMAGE_EXPORT_DIRECTORY *pIED =
      (IMAGE_EXPORT_DIRECTORY *)((DWORD)hUser32 + iedRVA);
  ...
  for (DWORD i = 0; i < pIED->NumberOfNames; ++i) {
    ...
    DWORD functionRVA =
        ((DWORD *)((DWORD)hUser32 + pIED->AddressOfFunctions))[ordinal];
    PVOID pProcAddress = (PVOID)((DWORD)hUser32 + functionRVA);

    PAPI_FUNCTION api = (PAPI_FUNCTION)VirtualAlloc(NULL, sizeof(PAPI_FUNCTION),
    ...
    DWORD thread_id;
    HANDLE hFuzzThread =
        CreateThread(NULL, 0, fuzz_proc, (PVOID)api, 0, &thread_id);
    if (hFuzzThread) {
